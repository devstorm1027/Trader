...
class pygem_rest(object):
    ''' A Python wrapper for the Gemini RESTful API '''
...
class pygem_socket(object):
    ''' A Python Wrapper for the Gemini Web Socket API. '''

    def __init__(self, key, secret_key, sandbox=True, debug=False):
        '''
        Creates a pygem_socket object, providing methods for the
        Gemini Web Socket API.
        ...
        sandbox (bool): Wheter to use the sandbox (True) or the live account.
            Default is True.
        debug (bool): If true, some additional output will be printed.
            Default is False.
        '''
        ...
        if sandbox:
            self.url = 'wss://api.sandbox.gemini.com/v1/'
        else:
            self.url = 'wss://api.gemini.com/v1/'
        self.debug = debug

    def make_order_status_streamer(self):
        return order_status_stream(self.url, self.key,
                                   self.secret_key, self.debug)

    def make_market_data_streamer(self, symbol):
        if symbol not in ['BTCUSD', 'ETHUSD', 'ETHBTC']:
            raise ValueError(
                "symbol must be one of 'BTCUSD', 'ETHUSD', 'ETHBTC'")
        url = self.url + 'marketdata/' + symbol
        return market_data_stream(url, self.books_ask[symbol],
                                  self.books_bid[symbol], self.debug)
...
class order_status_stream(object):
    def __init__(self, url, key, secret_key, debug=False):
    ...
    def on_message(self, ws, msg):
        ...
        if type(data) == dict:
            if self.debug:
                print('Got an %s event' % data['type'])
            ...
            for ev in data:
                if self.debug:
                    print('Received %s event' % ev['type'])
                if ev['type'] == 'initial':
    ...
    def set_callback(self, event, fkt):
        if event not in ['initial', 'accepted', 'rejected', 'booked',
            ...
            raise ValueError("event must be one of  'initial', 'accepted', \
                             'rejected', 'booked',\
                             'fill', 'cancelled', 'cancel_rejected', \
                             'closed', \
                             'heartbeat' or 'subscription_ack'")
        if not callable(fkt):
...
class market_data_stream(object):
    def __init__(self, url, book_ask, book_bid, debug):
        self.url = url
        self.ws = None
        self.wst = None
            ...
            for ev in data['events']:
                if ev['type'] == 'change':
                    ...
                    if ev['side'] == 'ask':
                        ...
                        book = self.book_bid
                    book[ev['price']] = {
                        'remaining': ev['remaining'], 'delta': ev['delta']}
                    ...
                    if ev['reason'] == 'initial':
                        ...
                        warnings.warn(
                            'Got an unknowns change event: %s' % ev['reason'])
    ...
    def set_callback(self, event, fkt):
        if event not in ['change', 'initial_change', 'place_change',
            ...
            raise ValueError("event must be one of 'change', 'initial_change', \
                             'place_change', 'trade_change', 'cancel_change', \
                             'trade', 'auction'")
        if not callable(fkt):
            raise ValueError('fkt is not callable')
        if event == 'change':
